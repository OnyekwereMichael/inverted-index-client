...
const Cart = () => {
  ...
  return (
    <div className="px-4 md:px-8 pt-6 pb-10 bg-gray-100 flex flex-col items-center">
      {/* <h2 className="text-2xl md:text-3xl font-bold text-blue-700 mb-6 text-center">YOUR CART</h2> */}
      {cart.length === 0 ? (
        <p className="text-center text-gray-600 text-lg">Your cart is empty 👀.</p>
      ) : (
        <div className="w-full  bg-white shadow-lg rounded-lg p-4 md:p-6 overflow-x-auto">
          <div className="hidden md:block">
          <table className="w-full border-collapse table-fixed">
  <thead className="bg-blue-600 text-white text-lg">
    <tr>
      <th className="p-3 w-2/5 text-left">Product</th>
      <th className="p-3 w-1/5 text-center">Price</th>
      <th className="p-3 w-1/5 text-center">Quantity</th>
  ...
  <tbody>
    {cart.map((item, index) => (
      <tr key={index} className="border-b hover:bg-gray-50">
        <td className="p-3 w-2/5 flex items-center gap-3 whitespace-nowrap">
          <img src={item.image} alt={item.name} className="w-16 h-16 object-cover rounded-lg border" />
          <p className="font-semibold text-gray-800">{item.name}</p>
        </td>
        <td className="p-3 w-1/5 text-center text-lg font-medium text-gray-700 whitespace-nowrap">
          ₦{item.price}
        </td>
        ...
        <td className="p-3 w-[100%] flex  gap-2">
          <button
